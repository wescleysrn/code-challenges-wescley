package com.iamandu.codechallenger.problems.codility.dynamicprogramming;

public interface NumberSolitaire {

	/**
	
	A game for one player is played on a board consisting of N consecutive squares, numbered 
	from 0 to N − 1. There is a number written on each square. A non-empty array A of N integers 
	contains the numbers written on the squares. Moreover, some squares can be marked during the game.
	...
	standing on square number I to square number I + K, providing that square number I + K exists. 
	...
	that, given a non-empty array A of N integers, returns the maximal result that can be achieved on the 
	board represented by array A.
